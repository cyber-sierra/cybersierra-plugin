...
```
cybersierra <module> <resource> <action> [flags]
```
...
Do not hardcode module lists. Query `cybersierra manifest --raw` and inspect the `tree` keys to discover available modules dynamically.
